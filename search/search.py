...
def depthFirstSearch(problem):
    ...
    fringe = util.Stack()
    startPath = []
    start = (problem.getStartState(), startPath)
    fringe.push(start)
    visited = []
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    fringe.push(start, costs[start[0]])
    visited = []
    ...
    while not fringe.isEmpty():
        ...
        if node not in visited:
            ...
            for nextState, action, stateCost in problem.getSuccessors(node):
                newPath = path + [action]
                costs[nextState] = problem.getCostOfActions(newPath) + heuristic(nextState, problem)
                fringe.push((nextState, newPath), costs[nextState])
