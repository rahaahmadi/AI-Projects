...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        # calculate distance from the closest food
        ...
        for ghost in successorGameState.getGhostPositions():
            # enforce agent to move away from ghosts if distance is less than 2
            if manhattanDistance(newPos, ghost) < 2:
                return float('-Inf')

        # The closer agent is to the closest pellet the larger the score
        return successorGameState.getScore() + 1.0/closestFoodDist
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        for action in gameState.getLegalActions(agentIndex=0):
            nextState = gameState.generateSuccessor(agentIndex=0, action=action)
            nextValue = self.getValue(nextState, currentDepth=0, agentIndex=1)
    ...
    def maxValue(self, gameState, currentDepth, agentIndex):
        ...
        for action in gameState.getLegalActions(agentIndex):
            maxValue = max(maxValue, self.getValue(gameState.generateSuccessor(agentIndex, action), currentDepth, agentIndex+1))
        return maxValue
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getValue(self, gameState, currentDepth, agentIndex):
        if currentDepth == self.depth or gameState.isWin() or gameState.isLose():
            ...
            return self.expectedValue(gameState, currentDepth, agentIndex)
    ...
    def expectedValue(self, gameState, currentDepth, agentIndex):
        avgValue = 0
...
def betterEvaluationFunction(currentGameState):
    ...
    # prioritize the winning state
    if currentGameState.isWin():
        return float("+Inf")

    # postpone the defeat state
    ...
    for foodPos in foodList:
        closestFoodDist = min(closestFoodDist, manhattanDistance(foodPos, pos))

    # encourages the agent to get closer to food
    score -= 2 * closestFoodDist

    # encourages the agent to eat food
    score -= 5 * len(foodList)

    # encourages the agent to move closer to the capsules
    capsulePos = currentGameState.getCapsules()
    score -= 5 * len(capsulePos)
